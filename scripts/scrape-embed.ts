...
async function extractDataFromMD(): Promise<Document[]> {
  try {
    const loader = new NotionLoader('/Users/zack/Documents/Project/github/langchain-supabase-website-chatbot/md/');
    const docs = await loader.load();
    return docs;
  } catch (error) {
    console.error(error);
...
async function embedDocuments(
  ...
  try{
  const a = await SupabaseVectorStore.fromDocuments(client, docs, embeddings);
  }catch(error){
    console.log(error);
  }
...
(async function run(urls: string[]) {
  try {
    ...
    const rawDocs = await extractDataFromMD();
